...
function DitheredWaves({
  ...
  useEffect(() => {
    ...
    // Update both the ref uniform and the active material uniform directly
    const resRef = waveUniformsRef.current.resolution.value;
    if (resRef.x !== w || resRef.y !== h) {
      resRef.set(w, h);
    }

    if (mesh.current && mesh.current.material && mesh.current.material.uniforms) {
      const resMat = mesh.current.material.uniforms.resolution.value;
      if (resMat.x !== w || resMat.y !== h) {
        resMat.set(w, h);
      }
    }
  }, [size, gl]);
